...
提示音太响或太轻？用 `volume` 配置项调节（**0-100**，0 = 完全静音，100 = 原始音量，默认 100）。

**配置位置**：打开 profile 的用户 patch 层
...
> 💡 **实现原理**：插件直接对 WAV 的 PCM 采样做振幅缩放（零依赖、跨平台），**不会改动你的系统音量**。缩放结果会缓存在系统临时目录，同一音量只计算一次。
>
...
plugins:
  dsh-perlica-ding:
    enabled: true        # 总开关
    volume: 100          # 音量 0-100（0=静音，100=原声）
